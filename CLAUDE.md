...
- Both LLM calls run on the Hugging Face Inference Providers router via `HF_TOKEN`, pinned inline as `LLM_ENDPOINT` / `LLM_MODEL` constants: `core.py` summarizes with `meta-llama/Llama-3.3-70B-Instruct:cheapest`; `evolve.py` mutates with the code-specialized `Qwen/Qwen2.5-Coder-32B-Instruct:cheapest`. Both use the OpenAI-compatible response shape (`choices[0].message.content`).
